...
class GlobeRenderer extends EventEmitter2 {
  ...
  init() {
    ...
    mc.on('panmove', this.onPanMove.bind(this));
    ...
    dom.addEventListener('mousewheel', this.onMouseWheel.bind(this), false);
...
class Globe extends Component {
  ...
  componentDidMount () {
    this.globeRenderer = new GlobeRenderer(this.container.current, isTouchDevice())
    ...
    if (this.props.thermodynamicChart) {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.value !== this.props.value) {
      if (nextProps.value) {
        this.globeRenderer.focusCountry(nextProps.value)
      } else {
        this.globeRenderer.clearCountryFocus()
      }
    }

    if (nextProps.countryPrice !== this.props.countryPrice) {
      this.renderPrice(nextProps.countryPrice)
    }

    if (nextProps.thermodynamicChart !== this.props.thermodynamicChart) {
      if (nextProps.thermodynamicChart) {
        this.globeRenderer.setPoints(this.pointSeries)
      } else {
        this.globeRenderer.remove()
      }
  ...
  renderPrice(priceMap2) {
    const priceMap = Object.freeze(priceMap2 || this.props.countryPrice)
    let maxPrice = 0;
    ...
    this.pointSeries = countryPointsJson.map(country => {
      const points = [];
